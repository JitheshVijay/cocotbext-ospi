...
def cell_value(sig, edge, width=8):
    ...
    if oe:
        ...
        if bits and all(c in "01" for c in bits):
            ...
            if lanes == 1:
                ...
                label = f"{value:02X}"      # a whole byte per clock
            else:
                label = f"{value:X}"
            return label, "M"
        return "", "M"
...
def comparison(sig, found, path):
    ...
    wave = {
        "signal": rows,
        "head": {"text": "One byte at every width "
...
def main():
    ...
    wanted = {
        0x8B: ("octal-read.svg",
               "Fast read octal I/O (0x8B): single-lane opcode, then eight lanes",
               [("opcode 0x8B (1 lane)", 8), ("addr", 3),
                ("mode", 1), ("dummy cycles", 8), ("data", 1)]),
